...
class CfnReturns(object):
    ...
    def gen_publicip(self):
        rndoct1 = str(randint(2, 254))
        rndoct2 = str(randint(2, 254))
        rndoct3 = str(randint(2, 254))
        return '123.%s.%s.%s' % (rndoct1, rndoct2, rndoct3)

    def get_ref(self, res, res_name='myresource', stack_name='mystack', region='us-east-1', accountid='123456789012'):
        return self.get_returns(res, res_name, stack_name, region)['Ref']
    ...
    def get_returns(self, res, res_name='myresource', stack_name='mystack', region='us-east-1', accountid='123456789012'):
        ...
        if res_type == 'AWS::ApiGateway::Account':
            ...
            return {
                'Ref': 'i-0%s' % (self.gen_hexchars(16)),
                'Fn::GetAtt': {
            ...
            return {
                ...
                'Fn::GetAtt': {
                    ...
                    'CanonicalHostedZoneNameID': "Z%s" % (self.gen_alnumchars(13, True)),
                    'DNSName': dnsname,
            ...
            return {
                'Ref': lambdaref,
                'Fn::GetAtt': {
                    'Arn': 'arn:aws:lambda:%s:%s:%s' % (region, accountid, lambdaref)
                }
            }
            ...
            return {
                'Ref': rdsclusterref,
                'Fn::GetAtt': {
                    'Endpoint.Address': '%s.%s.%s.rds.amazonaws.com' % (rdsclusterref, alt_clusterid, region),
                    'Endpoint.Port': '5439', # TODO: Fix me, I am dependant upon the property and engine
                    'ReadEndpoint.Address': '%s.%s.%s.rds.amazonaws.com' % (rdsclusterroref, alt_clusterid, region)
                }
            }
            ...
            return {
                'Ref': rdsinstanceref,
                'Fn::GetAtt': {
                    'Endpoint.Address': '%s.%s.%s.rds.amazonaws.com' % (rdsinstanceref, self.gen_alnumchars(12, False), region),
                    'Endpoint.Port': '3306' # TODO: Fix me, I am dependant upon the property and engine
                }
            }
            ...
            return {
                'Ref': bucketref,
                'Fn::GetAtt': {
                    'Arn': 'arn:aws:s3:::%s' % (bucketname),
                    'DomainName': '%s.s3.amazonaws.com' % (bucketref),
                    'DualStackDomainName': '%s.s3.dualstack.%s.amazonaws.com' % (bucketref, region),
                    'WebsiteURL': 'http://%s.s3-website-%s.amazonaws.com/' % (bucketref, region)
                }
            }
            ...
            return {
                'Ref': 'arn:aws:sns:%s:%s:%s-%s-%s' % (region, accountid, stack_name, topicname, self.gen_alnumchars(12, True)),
                'Fn::GetAtt': {
                    'TopicName': topicname
                }
            }
